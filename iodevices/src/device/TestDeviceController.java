...
public class TestDeviceController {
	...
	private JTextField txtCharacterToSend;
	...
	private void initialize() {
		...
		btnSendByte.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				deviceController.byteToDevice(DeviceController.CONSOLE_OUT, (byte) thisValue);
			}
		});
		btnSendByte.setBounds(10, 25, 129, 23);
		panelFromCPU.add(btnSendByte);

		JButton btnSendCharacter = new JButton("Send Character");
		btnSendCharacter.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				byte b ;
				char[] charsToSend = txtCharacterToSend.getText().toCharArray();
				for(Character c :charsToSend){
					b = (byte) c.charValue();
					deviceController.byteToDevice(DeviceController.CONSOLE_OUT, b);
				}//for
			}//actionPerformed
		});
		btnSendCharacter.setBounds(10, 59, 129, 23);
		panelFromCPU.add(btnSendCharacter);
		...
		txtCharacterToSend = new JTextField();
		txtCharacterToSend.setBounds(149, 60, 86, 20);
		panelFromCPU.add(txtCharacterToSend);
		txtCharacterToSend.setColumns(10);
		...
		ftfByteReceived.setBounds(150, 34, 40, 20);
		...
		JButton btnGetAllBytes = new JButton("Get Byte");
		...
		lblNotConnected.setHorizontalAlignment(SwingConstants.CENTER);
		lblNotConnected.setBounds(56, 292, 215, 23);
		frmTestDeviceController.getContentPane().add(lblNotConnected);
	}// initialize
}//
